...
{
	// 原始音频存放地址
	private static final String local_file = "D:/temp/desktop/yuyin/徐州工厂.mp3";
	
	private static final String Result_file = "D:/temp/desktop/yuyin/徐州工厂.txt";
...
//			lc.initLfasrClient(app_id, secret_key)
		...
		// 获取任务结果
		try {
			FileOutputStream fileOutputStream = new FileOutputStream(new File(Result_file));
			...
			if (resultMsg.getOk() == 0) {
				// 打印转写结果
				System.out.println(resultMsg.getData());
				JSONArray resultArray = JSONObject.parseArray(resultMsg.getData());
				for(int index=0;index<resultArray.size();index++){
					System.out.println(resultArray.getJSONObject(index).getString("onebest"));
					bufferedOutputStream.write(resultArray.getJSONObject(index).getString("onebest").getBytes());
